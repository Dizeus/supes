...
describe('testing Reducer', () => {
    ...
    test('set heroes', () => {
        expect(heroReducer({heroes: []}, setHeroes(heroes))).toEqual({heroes: heroes})
    })

    test('add hero', () => {
        expect(heroReducer({heroes: deleteHeroes}, addHero(heroToAdd))).toEqual({heroes: [...heroes].reverse()})
    })

    test('update hero', () => {
        expect(heroReducer({heroes: heroes}, updateHero(heroToUpdate))).toEqual({heroes: updatedHeroes})
    })

    test('delete hero', () => {
        expect(heroReducer({heroes: heroes}, deleteHero( "b4ad176c-c279-4498-a638-30ddb0baf6b4"))).toEqual({heroes: deleteHeroes})
    })
})